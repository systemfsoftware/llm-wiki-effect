...
const splitInclusive = (text: string, separator: string): ReadonlyArray<string> => {
  // Stryker disable next-line ArrayDeclaration,ConditionalExpression,StringLiteral
  if (text === '') return []
  const parts = text.split(separator)
  // Stryker disable next-line ArithmeticOperator,ConditionalExpression
...
const findFrontmatterClose = (rest: string): readonly [number, number] | undefined => {
  ...
  // Stryker disable next-line ArrayDeclaration,ConditionalExpression,MethodExpression,StringLiteral
...
const splitWithOverlap = (
  ...
  while (start < chars.length) {
    ...
    // Stryker disable next-line ConditionalExpression,MethodExpression,StringLiteral
...
const splitPreservingAtomicBlocks = (
  ...
  while (index < source.length) {
    ...
    if (line.trimStart().startsWith('|')) {
      flushNormal()
      const start = index
      // Stryker disable next-line ConditionalExpression,EqualityOperator,MethodExpression,StringLiteral
