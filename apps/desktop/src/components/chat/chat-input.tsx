...
export function ChatInput({
  ...
  const showSlashSkills = !!slashSkillTrigger &&
  ...
  const [prevSlashSkillKey, setPrevSlashSkillKey] = useState(slashSkillKey)
  const [prevContextFileKey, setPrevContextFileKey] = useState(contextFileKey)

  if (prevSlashSkillKey !== slashSkillKey) {
    setPrevSlashSkillKey(slashSkillKey)
    setSlashSkillIndex(0)
    setDismissedSlashKey(null)
  }

  if (prevContextFileKey !== contextFileKey) {
    setPrevContextFileKey(contextFileKey)
  ...
  return (
    <div className='border-t bg-background/95 p-3'>
      <div
        ref={inputFrameRef}
        className='relative rounded-lg border border-border/80 bg-card/80 p-2 shadow-sm ring-1 ring-black/5 focus-within:border-ring/60 focus-within:ring-ring/20 dark:ring-white/5'
      >
        {images.length > 0 && (
        ...
        {imageError && <p className='mb-1 px-1 text-xs text-destructive'>{imageError}</p>}
              ...
              {contextFileOptions.map((path, index) => (
                <button
                  ...
                  className={`flex w-full items-center gap-2 rounded px-2 py-1.5 text-left text-xs ${
                    index === contextFileIndex ? 'bg-accent' : 'hover:bg-accent/60'
                  }`}
                >
          ...
          <div className='flex min-w-0 flex-wrap items-center gap-1.5'>
            ...
            >
              ...
              <select
                value={retrievalMode}
                onChange={(event) => {
                  const mode = RETRIEVAL_MODE_OPTIONS.find((option) => option === event.target.value)
                  if (mode) onRetrievalModeChange(mode)
                }}
                disabled={isStreaming}
            ...
            >
              <select
                value={agentMode}
                onChange={(event) => {
                  const mode = AGENT_MODE_OPTIONS.find((option) => option === event.target.value)
