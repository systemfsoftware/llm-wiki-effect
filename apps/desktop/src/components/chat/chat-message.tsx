...
function AgentTurnActivity({
  ...
  return (
    <section
      ...
      aria-label={t('chat.agentChanges.taskTitle')}
    >
...
function SavedAgentActivity({
  ...
  return (
    <div
      className={embedded
        ? 'space-y-1 border-b border-border/40 px-2 py-1.5'
        : 'space-y-1 rounded-md border border-border/50 bg-background/50 px-2 py-1'}
    >
        ...
        <div className='grid gap-1.5'>
          {(field.options ?? []).map((option) => (
            ...
            >
              ...
              {option.description && <span className='mt-0.5 block text-[11px] opacity-80'>{option.description}</span>}
            </button>
          ))}
        </div>
      )}
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (hasUsableLlm(llmConfig)) {
        ...
        autoIngest(pp, filePath, llmConfig).catch((err) => console.error('Failed to auto-ingest saved query:', err))
      }
    } catch (err) {
...
function CitedReferencesPanel({
  ...
  useEffect(() => {
    let cancelled = false
    if (project && citedPages.length > 0) {
      const pp = normalizePath(project.path)
      void Promise.all(
        ...
        const infos: Record<string, CitedImageInfo> = {}
        if (!cancelled) {
          for (const [path, info] of entries) infos[path] = info
          setImageInfos(infos)
        }
        return infos
      })
  ...
  const handleJumpToImageSource = useCallback(
    async (firstUrl: string, fallbackPath: string) => {
      ...
      if (rawPath) {
        try {
          ...
          if (onOpenReferencePreview) {
            ...
            openFileInPreview(rawPath, rawContent)
          }
      ...
      try {
        ...
        const fallbackContent = await readFile(fallbackAbsPath)
  ...
  const openCitedPage = useCallback(async (page: CitedPage) => {
    if (page.kind === 'workspace') {
      ...
      try {
        ...
        const workspaceContent = shouldReadContent ? await readFile(workspacePath) : ''
        if (onOpenReferencePreview) {
          onOpenReferencePreview({
            title: page.title,
            path: workspacePath,
    ...
    if (page.kind === 'external') {
      ...
      const previewPath = `${isAnyTxtReference(page) ? 'anytxt' : 'external'}-preview://${
    ...
    for (const candidate of candidates) {
      try {
        const pageContent = await readFile(candidate)
        if (onOpenReferencePreview) {
          onOpenReferencePreview({
            title: page.title,
            path: candidate,
          ...
          openFileInPreview(candidate, pageContent)
        }
        return
      } catch {
        // try next
            ...
            {visiblePages.map((page, i) => {
              ...
              return (
                ...
                <div
                  ...
                  title={page.kind === 'external'
                    ? `${referenceSourceLabel(page)}: ${referenceLocator(page)}`
                    : page.path}
                >
  ...
  return (
    <div className='flex gap-2 flex-row'>
      ...
      <div className='max-w-[80%] rounded-lg px-3 py-2 text-sm bg-muted text-foreground'>
        ...
        {isThinking ? <StreamingThinkingBlock content={thinking} /> : (
          <>
    ...
    <div className='rounded-md border border-dashed border-amber-500/30 bg-amber-50/50 dark:bg-amber-950/20 px-2.5 py-2'>
      <div className='flex items-center gap-1.5 mb-1.5'>
        ...
        <span className='text-[10px] text-amber-600/50 dark:text-amber-500/40'>
          {t('chat.lineCount', { count: lines.length })}
        </span>
      </div>
