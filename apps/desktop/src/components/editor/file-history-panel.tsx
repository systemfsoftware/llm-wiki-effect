import { type FileHistoryEntry, listFileHistory, restoreFileHistory } from '@/commands/fs'
...
export function FileHistoryButton({ filePath, currentContent }: { filePath: string; currentContent: string }) {
  ...
  const diff = selected
    ? summarizeAgentFileChange({
      ...
      path: filePath,
      tool: selected.tool,
      beforeContent: selected.content,
  ...
  return (
    <>
      <button
        type='button'
        onClick={() => void show()}
        className='absolute right-3 top-3 z-20 rounded-md border bg-background/90 p-1.5 text-muted-foreground shadow-sm hover:text-foreground'
        ...
        <div className='absolute inset-0 z-40 flex bg-background'>
          <aside className='w-72 shrink-0 border-r p-3'>
            <div className='mb-3 flex items-center justify-between'>
              <strong className='text-sm'>{t('preview.history')}</strong>
            ...
            <div className='space-y-1 overflow-auto'>
              ...
              {!loading && entries.length === 0 && (
                <p className='text-xs text-muted-foreground'>{t('preview.historyEmpty')}</p>
              )}
              {entries.map((entry) => (
                <button
                  key={entry.id}
                  ...
                  className={`w-full rounded border px-2 py-2 text-left text-xs ${
                    selected?.id === entry.id ? 'border-primary bg-primary/5' : 'border-transparent hover:bg-muted'
                  }`}
                >
            ...
            {selected
              ? (
                <>
                  ...
                  <pre className='whitespace-pre-wrap break-words rounded-md bg-muted/40 p-3 font-mono text-xs'>{diff}</pre>
                </>
              )
              : (
                <div className='grid h-full place-items-center text-sm text-muted-foreground'>
                  {t('preview.historySelect')}
                </div>
              )}
          </main>
        </div>
