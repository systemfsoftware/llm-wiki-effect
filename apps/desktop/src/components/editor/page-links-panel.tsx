...
export function PageLinksPanel({ filePath, onClose }: { filePath: string; onClose: () => void }) {
  ...
  const loadKey = [project?.path ?? '', filePath, dataVersion].join('\0')
  ...
  useEffect(() => {
    ...
    const key = [project.path, filePath, dataVersion].join('\0')
    let cancelled = false
    ...
    const timer = window.setTimeout(() => {
      ...
      void (async () => {
        try {
          const result = await getPageLinks(project.path, filePath)
  ...
  return (
    <aside
      className='absolute inset-y-0 right-0 z-20 flex w-[min(22rem,90%)] flex-col border-l border-border bg-background shadow-xl'
      aria-label={t('editor.pageLinks.title')}
    >
        ...
        {loading
          ...
          : (
            <>
              <LinkSection
                icon={Link2}
                title={t('editor.pageLinks.outgoing')}
                entries={links.outgoing}
                empty={t('editor.pageLinks.noOutgoing')}
                onOpen={openEntry}
              />
              <LinkSection
                icon={CornerUpLeft}
                title={t('editor.pageLinks.backlinks')}
                entries={links.backlinks}
                empty={t('editor.pageLinks.noBacklinks')}
                onOpen={openEntry}
              />
              <LinkSection
                icon={FileQuestion}
                title={t('editor.pageLinks.missing')}
                entries={links.missing}
                empty={t('editor.pageLinks.noMissing')}
                onOpen={openEntry}
...
function LinkSection({
  ...
  return (
    <section className='border-b border-border'>
      ...
      {entries.length === 0 ? <p className='px-3 py-3 text-xs text-muted-foreground'>{empty}</p> : (
              ...
              <div key={`${entry.title}:${index}`} className='border-t border-border/50 px-3 py-2 first:border-t-0'>
                ...
                <div className='mt-1.5 flex gap-1'>
                  <button
                    ...
                    onClick={() => void onCreate?.(entry, false)}
                    className='inline-flex items-center gap-1 border border-border px-1.5 py-1 text-[10px] text-foreground hover:bg-accent disabled:opacity-50'
                  >
                    <Plus className='h-3 w-3' />
                    {t('editor.pageLinks.create')}
                  </button>
                  <button
                    type='button'
                    disabled={Boolean(creatingTitle)}
                    onClick={() => void onCreate?.(entry, true)}
                    className='inline-flex items-center gap-1 bg-primary px-1.5 py-1 text-[10px] text-primary-foreground disabled:opacity-50'
                  >
                    {creatingTitle === entry.title
                      ? <Loader2 className='h-3 w-3 animate-spin' />
                      : <Sparkles className='h-3 w-3' />} {t('editor.pageLinks.draft')}
                  </button>
                </div>
              </div>
            )
