...
import { type SearchResult, searchWiki } from '@/lib/search'
import {
  buildWordDiff,
  findDomTextSelection,
  findUniqueTextSelection,
  normalizeEditableMarkdown,
  normalizeSelectionReplacement,
} from '@/lib/selection-edit'
import { useWikiStore } from '@/stores/wiki-store'
import 'katex/dist/katex.min.css'
import { Eye, Link2, Pencil, Sparkles, X } from 'lucide-react'
...
export function WikiEditor({ content, onSave, filePath }: WikiEditorProps) {
  ...
  const submitSelectionToAgent = useCallback(async (intent: 'ask' | 'edit') => {
    ...
    if (selectionRequest.sourceMapped) {
      onSave(`${selectionRequest.prefix}${selectionRequest.selectedText}${selectionRequest.suffix}`, {
        immediate: true,
      })
    }
    ...
    const retrievedContext = references.length > 0
      ? references.map((reference, index) =>
        `[${index + 1}] ${reference.title}\nPath: ${
          projectRelativePath(projectRoot, reference.path)
        }\n${reference.snippet}`
      ).join('\n\n')
    ...
    const prompt = [
      ...
      selectionRequest.sourceMapped
        ? selectionRequest.prefix.slice(-1200)
        : 'Unavailable because the rendered selection could not be mapped safely to one source range.',
      '<selected_text>',
      selectionRequest.selectedText,
      '</selected_text>',
      'Context after selection:',
      selectionRequest.sourceMapped
        ? selectionRequest.suffix.slice(0, 1200)
        : 'Unavailable because the rendered selection could not be mapped safely to one source range.',
  ...
  const openCitation = useCallback(async (reference: SearchResult) => {
    try {
      setSelectionError('')
      const citationContent = await readFile(reference.path)
      setCitationPreview({ path: reference.path, title: reference.title, content: citationContent })
    } catch (error) {
  ...
  return (
    ...
    >
        ...
        <div className='absolute right-3 top-3 z-10 flex items-center gap-1'>
          {filePath && (
            <button
              ...
              onClick={() => {
                setSelectionRequest(null)
                setShowPageLinks((shown) => !shown)
              }}
              title={t('editor.pageLinks.title')}
      ...
      {selectionRequest && (
        <aside
          className='flex h-full w-[360px] max-w-[45%] shrink-0 flex-col border-l border-border bg-background'
          aria-label={t('editor.selection.askAgent')}
        >
          ...
          <div className='flex-1 overflow-y-auto p-3'>
            ...
            {selectionTurns.map((turn, index) => (
              <div key={`${index}:${turn.question}`} className='mb-4 space-y-2 border-b border-border/60 pb-4'>
                <div className='ml-6 rounded-md bg-accent px-2.5 py-1.5 text-xs text-accent-foreground'>
                  {turn.question}
                </div>
                  ...
                  <div className='flex flex-wrap gap-1 pt-1'>
                    {turn.references.map((reference, referenceIndex) => (
                      <button
                        key={reference.path}
                        type='button'
                        onClick={() => void openCitation(reference)}
                        title={reference.path}
                        className='max-w-full truncate rounded border border-border px-1.5 py-1 text-[10px] text-muted-foreground hover:bg-accent hover:text-foreground'
                      >
            ...
            {citationPreview && (
              <section className='mb-4 overflow-hidden rounded-md border border-border bg-background'>
                <div className='flex items-center gap-2 border-b border-border px-2 py-1.5'>
                  ...
                  >
                    {t('editor.selection.closeReference')}
                  </button>
                </div>
                <div className='max-h-72 overflow-auto p-2'>
                  <WikiReader body={parseFrontmatter(citationPreview.content).body} filePath={citationPreview.path} />
                </div>
              </section>
            )}
            {selectionResult && (
              <div className='space-y-3'>
                {selectionResult.intent === 'edit' && (
                  <div className='rounded-md border border-border bg-muted/20 p-2'>
                    <div className='mb-1 text-[10px] font-semibold uppercase text-muted-foreground'>
                      {t('editor.selection.wordDiff')}
                    </div>
                    <div className='whitespace-pre-wrap break-words font-mono text-xs leading-5'>
                      {buildWordDiff(
                        selectionRequest.selectedText,
                        normalizeSelectionReplacement(selectionResult.content),
                      ).map((part, index) => (
                        <span
                          key={`${index}:${part.type}`}
                          className={part.type === 'delete'
                ...
                <div
                  className={selectionResult.intent === 'edit'
                    ? 'rounded-md border border-border bg-muted/20 p-2'
                    : 'min-w-0 text-sm text-foreground'}
                >
                  {selectionResult.intent === 'edit' && (
                    <div className='mb-1 text-[10px] font-semibold uppercase text-muted-foreground'>
                      + {t('chat.selectionEdit.replacement')}
                    </div>
                  )}
                  {selectionResult.intent === 'edit'
                    ? (
                      selectionRunning
                        ? (
                          <div className='whitespace-pre-wrap break-words'>
                            {selectionResult.content || t('editor.selection.generating')}
                          </div>
                        )
                        : (
                          <textarea
                            value={selectionResult.content}
                            onChange={(event) => setSelectionResult({ intent: 'edit', content: event.target.value })}
                            rows={8}
                            className='w-full resize-y bg-transparent font-mono text-xs leading-5 outline-none'
                          />
                        )
                    )
              ...
              <textarea
                ...
                onKeyDown={(event) => {
                  if (event.key === 'Enter' && !event.shiftKey) {
                    event.preventDefault()
                    void submitSelectionToAgent(selectionMode ?? 'ask')
                  }
                }}
              ...
              {selectionRunning
                ...
                ? (
                  <>
                    ...
                    >
                      {t('chat.selectionEdit.reject')}
                    </button>
                    <button
                      type='button'
                      onClick={() => void submitSelectionToAgent('edit')}
                      className='rounded-md border border-border px-2 py-1 text-xs text-foreground hover:bg-accent'
                    >
                      {t('editor.selection.regenerate')}
                    </button>
                    <button
                      type='button'
                      onClick={() => void acceptSelectionEdit()}
                      disabled={!selectionResult.content}
                      className='rounded-md bg-primary px-2 py-1 text-xs text-primary-foreground disabled:opacity-50'
                    >
                      {t('chat.selectionEdit.accept')}
                    </button>
                  </>
                )
                : !selectionResult
                ? (
                  <>
                    {selectionMode !== 'edit' && (
                      <button
                        type='button'
                        onClick={() => void submitSelectionToAgent('ask')}
                        disabled={!selectionInstruction.trim()}
                        className='rounded-md border border-border px-2 py-1 text-xs text-foreground hover:bg-accent disabled:opacity-50'
                      >
                    ...
                    {selectionMode !== 'ask' && (
                      <button
                        type='button'
                        onClick={() => void submitSelectionToAgent('edit')}
                        disabled={!selectionInstruction.trim() || !selectionRequest.sourceMapped}
                        title={!selectionRequest.sourceMapped ? t('editor.selection.askOnlyHint') : undefined}
                        className='rounded-md bg-primary px-2 py-1 text-xs text-primary-foreground disabled:opacity-50'
                      >
