...
export function MermaidDiagram({ code }: MermaidDiagramProps) {
  return <MermaidDiagramRenderer key={code} code={code} />
}

function MermaidDiagramRenderer({ code }: MermaidDiagramProps) {
  ...
  const renderedSvg = svg ?? svgCache.get(code) ?? null

  // Only render when the diagram scrolls into view
  useEffect(() => {
    if (svgCache.has(code)) return undefined
  ...
  useEffect(() => {
    if (!visible || svgCache.has(code)) return undefined
    let cancelled = false
      ...
      <div className='my-2 max-w-full rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-xs text-destructive'>
        <div className='flex items-start gap-2'>
          ...
          <div className='min-w-0 flex-1'>
            <p className='font-medium'>{t('mermaid.renderFailed')}</p>
  ...
  return (
    <>
      <div
        ...
        style={{ minHeight: renderedSvg ? undefined : estimatedHeight }}
      >
        {renderedSvg
          ? (
            <>
          ...
          ? (
            <div className='flex items-center justify-center h-full p-4'>
              <span className='text-xs text-muted-foreground animate-pulse'>{t('mermaid.rendering')}</span>
            </div>
          )
          : (
            <div className='flex items-center justify-center h-full p-4'>
              <span className='text-xs text-muted-foreground/50'>{t('mermaid.diagram')}</span>
            </div>
          )}
      </div>

      {/* Fullscreen overlay */}
      {expanded && renderedSvg && (
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm'>
          ...
          />
          <div className='relative h-[90vh] w-[90vw] overflow-auto rounded-lg bg-background border border-border shadow-2xl p-6'>
