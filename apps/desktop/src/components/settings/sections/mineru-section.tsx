...
import type { MineruEffort, MineruLocalBackend, MineruModelVersion, MineruParseMethod } from '@/stores/wiki-store'
...
const MINERU_MODEL_VERSIONS: Record<MineruModelVersion, true> = { pipeline: true, vlm: true }
const MINERU_LOCAL_BACKENDS: Record<MineruLocalBackend, true> = {
  pipeline: true,
  'vlm-engine': true,
  'hybrid-engine': true,
  'vlm-http-client': true,
  'hybrid-http-client': true,
}
const MINERU_PARSE_METHODS: Record<MineruParseMethod, true> = { auto: true, txt: true, ocr: true }
const MINERU_EFFORTS: Record<MineruEffort, true> = { medium: true, high: true }

function isMineruModelVersion(value: string): value is MineruModelVersion {
  return Object.hasOwn(MINERU_MODEL_VERSIONS, value)
}

function isMineruLocalBackend(value: string): value is MineruLocalBackend {
  return Object.hasOwn(MINERU_LOCAL_BACKENDS, value)
}

function isMineruParseMethod(value: string): value is MineruParseMethod {
  return Object.hasOwn(MINERU_PARSE_METHODS, value)
}

function isMineruEffort(value: string): value is MineruEffort {
  return Object.hasOwn(MINERU_EFFORTS, value)
}
        ...
        <div className='space-y-4 pl-1'>
              ...
              <div className='space-y-2'>
                ...
                <p className='text-xs text-muted-foreground'>
                  {t('settings.sections.mineru.localInfo', {
                    defaultValue: 'Base URL of an official mineru-api or mineru-router service.',
                  })}
                </p>
              </div>
              <div className='space-y-2'>
            ...
            <div className='space-y-2'>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (isMineruModelVersion(value)) setDraft('mineruModelVersion', value)
                }}
              >
                <option value='vlm'>
              ...
              <p className='text-xs text-muted-foreground'>
                {t('settings.sections.mineru.modelHint', {
                  defaultValue:
                    'PDF parsing supports pipeline and vlm. MinerU-HTML is for HTML files and is not used here.',
                })}
              </p>
            </div>
          )}
            ...
            <div className='space-y-4'>
              <div className='space-y-2'>
                ...
                <select
                  ...
                  onChange={(e) => {
                    const value = e.target.value
                    if (isMineruLocalBackend(value)) setDraft('mineruLocalBackend', value)
                  }}
                >
              ...
              {draft.mineruLocalBackend.startsWith('hybrid-') && (
                <div className='space-y-2'>
                  ...
                  <select
                    id='mineru-effort'
                    className='w-full rounded-md border bg-background px-3 py-2 text-sm'
                    value={draft.mineruLocalEffort}
                    onChange={(e) => {
                      const value = e.target.value
                      if (isMineruEffort(value)) setDraft('mineruLocalEffort', value)
                    }}
                  >
              ...
              {(draft.mineruLocalBackend === 'pipeline' || draft.mineruLocalBackend.startsWith('hybrid-')) && (
                <div className='grid gap-4 sm:grid-cols-2'>
                  <div className='space-y-2'>
                    ...
                    <Input
                      id='mineru-language'
                      value={draft.mineruLocalLanguage}
                      onChange={(e) => setDraft('mineruLocalLanguage', e.target.value)}
                      placeholder='ch'
                    />
                  </div>
                  <div className='space-y-2'>
                    ...
                    <select
                      id='mineru-parse-method'
                      className='w-full rounded-md border bg-background px-3 py-2 text-sm'
                      value={draft.mineruLocalParseMethod}
                      onChange={(e) => {
                        const value = e.target.value
                        if (isMineruParseMethod(value)) setDraft('mineruLocalParseMethod', value)
                      }}
                    >
              ...
              {draft.mineruLocalBackend.endsWith('http-client') && (
                <div className='space-y-2'>
                  ...
                  <Input
                    id='mineru-server-url'
                    type='url'
                    value={draft.mineruLocalServerUrl}
                    onChange={(e) => setDraft('mineruLocalServerUrl', e.target.value)}
              ...
              <div className='flex flex-wrap gap-4 text-sm'>
                <label className='flex items-center gap-2'>
                  <input
                    type='checkbox'
                    checked={draft.mineruLocalFormulaEnabled}
                    onChange={(e) => setDraft('mineruLocalFormulaEnabled', e.target.checked)}
                  />
                  {t('settings.sections.mineru.formulaParsing')}
                </label>
                <label className='flex items-center gap-2'>
                  <input
                    type='checkbox'
                    checked={draft.mineruLocalTableEnabled}
                    onChange={(e) => setDraft('mineruLocalTableEnabled', e.target.checked)}
                  />
                  {t('settings.sections.mineru.tableParsing')}
                </label>
                <label className='flex items-center gap-2'>
                  <input
                    type='checkbox'
                    checked={draft.mineruLocalImageAnalysis}
                    onChange={(e) => setDraft('mineruLocalImageAnalysis', e.target.checked)}
                  />
                  {t('settings.sections.mineru.imageAnalysis')}
                </label>
              </div>
            </div>
          )}
          ...
          <div className='flex items-center gap-3'>
            <Button
              ...
              size='sm'
              onClick={handleTest}
              disabled={(draft.mineruBackend === 'cloud' && !draft.mineruToken.trim()) ||
