...
export function SkillsSection() {
  ...
  const persist = useCallback(async (nextSelected: string[], nextDisabled: string[]) => {
    ...
    await saveChatPreferences(projectPath, {
      useWebSearch,
      useAnyTxtSearch,
      agentMode,
      retrievalMode,
  ...
  }, [agentMode, projectPath, retrievalMode, useAnyTxtSearch, useWebSearch])
  ...
  }, [persist, projectPath, setSelectedSkills, t])
    ...
    <div className='space-y-6'>
      <div>
        ...
        <p className='mt-1 text-sm text-muted-foreground'>
          {t('settings.sections.skills.description', {
            defaultValue: 'Scan project and user skill folders, then choose which skills can be used from Chat.',
          })}
        </p>
      </div>
      ...
      <div className='rounded-md border bg-muted/20 p-3 text-xs text-muted-foreground'>
        {t('settings.sections.skills.paths', {
          defaultValue: 'Scanned folders: .llm-wiki/skills, ~/.claude/skills, ~/.codex/skills, ~/.agents/skills.',
        })}
      </div>
