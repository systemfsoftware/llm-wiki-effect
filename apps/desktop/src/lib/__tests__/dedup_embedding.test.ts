...
import { candidatePairs, clusterByPairs, cosineSimilarity, type Page, pageToEmbeddingText } from '../dedup_embedding'
import type { fetchEmbedding as fetchEmbeddingCommand } from '../embedding'

const { mockFetchEmbedding } = vi.hoisted(() => ({
  mockFetchEmbedding: vi.fn<typeof fetchEmbeddingCommand>(),
}))

vi.mock('../embedding', () => ({ fetchEmbedding: mockFetchEmbedding }))
...
const defaultEmbedding = async (text: string): Promise<number[] | null> => {
  const dim = 64
...
mockFetchEmbedding.mockImplementation(defaultEmbedding)
...
describe('candidatePairs', () => {
  ...
  it('skips pages whose embedding returns null', async () => {
    ...
    mockFetchEmbedding.mockImplementationOnce(async () => null) // p1
    mockFetchEmbedding.mockImplementationOnce(async () => null) // p2
    mockFetchEmbedding.mockImplementationOnce(async () => [1, 0]) // p3 axis 0
    mockFetchEmbedding.mockImplementationOnce(async () => [1, 0]) // p3 dup axis 0
  ...
  it('throws when too few pages embed successfully', async () => {
    mockFetchEmbedding.mockImplementation(async () => null)
