...
describe('MiniMax Provider', () => {
  ...
  it('enables streaming', () => {
    const cfg = getProviderConfig(makeConfig())
    expect(cfg.buildBody([])).toMatchObject({ stream: true })
  })
  ...
  it('includes max_tokens derived from context budget (required by Anthropic wire)', () => {
    ...
    expect(cfg.buildBody([])).toMatchObject({ max_tokens: 10240 })
  })

  it('carries the model in the body', () => {
    const cfg = getProviderConfig(makeConfig({ model: 'MiniMax-M3' }))
    expect(cfg.buildBody([])).toMatchObject({ model: 'MiniMax-M3' })
  })
...
describe('Anthropic output budget', () => {
  ...
  it('applies the derived default to native and custom Anthropic providers', () => {
    ...
    expect(native.buildBody([])).toMatchObject({ max_tokens: 10_240 })
    expect(custom.buildBody([])).toMatchObject({ max_tokens: 10_240 })
  })

  it('preserves an explicit caller override', () => {
    const provider = getProviderConfig(makeConfig({ provider: 'anthropic' }))

    expect(provider.buildBody([], { max_tokens: 777 })).toMatchObject({ max_tokens: 777 })
  })
})
...
describe('Sampling override translation across wires', () => {
  ...
  it('Gemini omits generationConfig entirely when no overrides passed', () => {
    ...
    expect(cfg.buildBody(baseMessages)).not.toHaveProperty('generationConfig')
  })
  ...
  it('OpenAI wires put overrides at the top level', () => {
    ...
    expect(cfg.buildBody(baseMessages, { temperature: 0.1, max_tokens: 500 }))
      .toMatchObject({ temperature: 0.1, max_tokens: 500 })
  })
  ...
  it('OpenAI o-series models use max_completion_tokens', () => {
    ...
    const body = cfg.buildBody(baseMessages, { max_tokens: 8192 })

    expect(body).toMatchObject({ max_completion_tokens: 8192 })
    expect(body).not.toHaveProperty('max_tokens')
  })
  ...
  it('custom OpenAI-compatible GPT-5 routes keep legacy overrides for OpenRouter-style shims', () => {
    ...
    const body = cfg.buildBody(baseMessages, { temperature: 0.1, max_tokens: 500 })

    expect(body).toMatchObject({ temperature: 0.1, max_tokens: 500 })
  ...
  it('custom Kimi routes strip unsupported temperature overrides', () => {
    ...
    const body = cfg.buildBody(baseMessages, { temperature: 0.1, max_tokens: 4096 })
