...
import type { AnyTxtConfig, LlmConfig, SearchApiConfig } from '@/stores/wiki-store'
...
type WebSearchFn = (query: string, config: SearchApiConfig, maxResults?: number) => Promise<WebSearchResult[]>
  ...
  it('resolves the source review only after a saved result exists', () => {
    ...
    useResearchStore.setState({
      tasks: [{
        ...
        webResults: [],
        synthesis:
          'This completed synthesis contains enough substantive analysis to explain the evidence, its limitations, and the conclusions that can reasonably be drawn. It also distinguishes established findings from open questions so the saved research is useful for later review.',
...
describe('collectResearchSources', () => {
  it('uses only Web Search when source mode is web', async () => {
    const webSearch = vi.fn<WebSearchFn>().mockResolvedValue([webResult])
    const anyTxtSearch = vi.fn<AnyTxtSearchFn>().mockResolvedValue([localResult])
  ...
  it('uses both sources concurrently and deduplicates by URL', async () => {
    ...
    const webSearch = vi.fn<WebSearchFn>().mockResolvedValue([webResult])
    const anyTxtSearch = vi.fn<AnyTxtSearchFn>().mockResolvedValue([duplicate, localResult])
  ...
  it('keeps web results when AnyTXT fails and exposes the source error', async () => {
    const webSearch = vi.fn<WebSearchFn>().mockResolvedValue([webResult])
    const anyTxtSearch = vi.fn<AnyTxtSearchFn>().mockRejectedValue(new Error('Check that ATGUI.exe is running'))
  ...
  it('logs once when research sources are capped', async () => {
    ...
    const webSearch = vi.fn<WebSearchFn>().mockResolvedValue(
    ...
    const anyTxtSearch = vi.fn<AnyTxtSearchFn>().mockResolvedValue([])
