...
type ChunkRow = { chunk_index: number; chunk_text: string; heading_path: string; embedding: number[] }
type EmbeddingTestConfig = typeof cfg & {
...
const mockInvoke = vi.fn<(cmd: string, args?: InvokeArgs) => Promise<unknown>>()
const mockEmbeddingFetchInvoke = vi.fn<(args?: InvokeArgs) => Promise<unknown>>()
...
const mockHttpFetch = vi.fn<(url: string, opts: RecordedFetchInit) => Promise<Response>>()
...
beforeEach(() => {
  ...
  mockEmbeddingFetchInvoke.mockImplementation((args) => {
    if (args === undefined || args.text === undefined || args.cfg === undefined) {
      throw new Error('embedding_fetch called without a text/cfg payload')
    }
    return fetchEmbeddingViaMockHttp(args.text, args.cfg, args.maxRetries ?? 3)
  })
  resetEmbeddingOptimizeAccountingForTests()
})
...
async function fetchEmbeddingViaMockHttp(
  ...
  for (const [name, value] of Object.entries(config.extraHeaders ?? {})) {
    ...
    if (['authorization', 'content-type', 'host', 'content-length', 'origin', 'x-goog-api-key'].includes(lower)) {
      continue
    }
    if (!value.trim()) continue
    headers[trimmed] = value.trim()
  }
  ...
  while (attempts <= maxRetries) {
    ...
    if (looksLikeOversizeForTest(response.status, textBody)) {
      throw new Error(
        `Endpoint rejected input even at ${current.length} chars — server context smaller than expected. Lower Settings → Embedding → Max Chunk Chars.`,
      )
    }
    throw new Error(
      `API ${response.status} ${response.statusText}${textBody ? ` — ${textBody.slice(0, 200)}` : ''} at ${endpoint}`,
    )
  }
...
function volcengineEndpointForTest(config: { endpoint: string; model: string }): string {
  ...
  const suffix = config.model.toLowerCase().includes('doubao-embedding-vision')
...
describe('searchByEmbedding — aggregation', () => {
  ...
  it('over-fetches topK × 3 chunks with a floor of 30 so the page-grouping has enough candidates', async () => {
    ...
    expect(searchCall, 'vector_search_chunks call').toBeDefined()
    expect(searchCall?.[1]?.topK).toBe(30)
    ...
    expect(searchCall2, 'vector_search_chunks call').toBeDefined()
    expect(searchCall2?.[1]?.topK).toBe(60)
  })
...
describe('fetchEmbedding — provider wire formats', () => {
  ...
  it('preserves non-key query parameters on pasted Gemini endpoints', async () => {
    ...
    await fetchEmbedding('hello', {
      enabled: true,
      endpoint:
        'https://generativelanguage.googleapis.com/v1beta/models/gemini-embedding-2:embedContent?key=url-key&other=foo',
      apiKey: 'header-key',
...
describe('embedAllPages', () => {
  ...
  it('uses OpenAI-compatible batch requests and preserves chunk order', async () => {
    ...
    mockInvoke.mockImplementation(async (command, args) => {
      if (command === 'embedding_fetch_batch') {
        ...
        return texts.map((_, index) => [index + 0.25])
      }
      return undefined
    })
    ...
    expect(batchCalls.every((texts) => texts.length <= 4)).toBe(true)
    const upsert = mockInvoke.mock.calls.find((call) => call[0] === 'vector_upsert_chunks')
    const chunks = upsert?.[1]?.chunks ?? []
  ...
  it('indexes every non-structural .md file, recursing into subdirs', async () => {
    ...
    const pageIds = upsertCalls.map((c) => c[1]?.pageId ?? '').sort()
