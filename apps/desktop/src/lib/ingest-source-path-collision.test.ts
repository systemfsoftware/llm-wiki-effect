...
function promptContent(messages: ChatMessage[] | undefined, index: number): string {
  const content = messages?.[index]?.content
  return typeof content === 'string' ? content : ''
}

vi.mock('./llm-client', () => ({
  streamChat: vi.fn<typeof streamChat>(async (_cfg, messages, cb) => {
    const systemPrompt = promptContent(messages, 0)
    const userPrompt = promptContent(messages, 1)
    ...
    const sourceIdentity = systemPrompt.match(/original source file is:\s*\*\*([^*]+)\*\*/i)?.[1] ?? 'config.yaml'
    cb.onToken([
      `---FILE: ${targetPath}---`,
      '---',
...
vi.mock('./mineru', () => ({
  parseWithMineru: vi.fn<typeof parseWithMineruResult>(),
  parseWithMineruResult: vi.fn<typeof parseWithMineruResult>(),
}))

import { autoIngest, buildFallbackSourceSummary, executeIngestWrites, hasMineruImageRefs } from './ingest'
...
describe('autoIngest source summary paths', () => {
  ...
  it('moves the canonical source summary and its source reference', async () => {
    ...
    await expect(fs.access(oldSummary)).rejects.toThrow(/ENOENT/)
  ...
  it('migrates a safe legacy basename source summary to the canonical nested source path', async () => {
    ...
    await expect(fs.access(legacySummaryPath)).rejects.toThrow(/ENOENT/)
  ...
  it('analyzes oversized sources in chunks before final wiki generation', async () => {
    ...
    const generationCall = mockStreamChat.mock.calls.find(([, messages]) =>
      promptContent(messages, 0).includes('Based on the analysis provided, generate wiki files')
    )
    expect(generationCall).toBeTruthy()
    const generationPrompt = promptContent(generationCall?.[1], 1)
  ...
  it('retries a truncated FILE block with a targeted generation request', async () => {
    ...
    expect(
      mockStreamChat.mock.calls.some(([, messages]) =>
        promptContent(messages, 0).startsWith(
  ...
  it('falls back to built-in PDF extraction when MinerU fails for a non-cancelled ingest', async () => {
    ...
    expect(
      updateSpy.mock.calls.some(([, updates]) => updates.detail?.includes('falling back to built-in PDF extraction')),
    ).toBe(true)
  ...
  it('canonicalizes interactive source summary paths and sources frontmatter', async () => {
    ...
    await expect(fs.access(staleSummaryPath)).rejects.toThrow(/ENOENT/)
    expect(content).toContain('sources: ["project-a/config.yaml"]')
  })
  ...
  it('rejects unsafe and application-managed paths from interactive writes', async () => {
    ...
    await expect(fs.access(path.join(tmp.path, 'escape.md'))).rejects.toThrow(/ENOENT/)
  })
})
