...
import { type OutputLanguage, useWikiStore } from '@/stores/wiki-store'
...
const scenarios: RealIngestScenario[] = [
  ...
  // ── C. Review-triggering (3) ────────────────────────────────────────────
  {
    ...
    description: 'Vision Transformer mentions concepts NOT in seed wiki ' +
  ...
  },
  {
    ...
    description: 'Attention deep-dive that paraphrases seed wiki attention.md heavily. ' +
  ...
  },
  {
    ...
    description: 'Claims attention uses Gaussian kernels rather than softmax. ' +
  ...
  // ── D. Knowledge graph / entity (2) ─────────────────────────────────────
  {
    ...
    description: 'Hinton biography: many entities (Turing Award, LeCun, Bengio, ' +
  ...
  },
  {
    ...
    description: 'Interconnected survey mentioning many wiki concepts (attention, ' +
  ...
  },
  {
    ...
    description:
      "Maxwell's equations with dense LaTeX. Greek letters leaking through must NOT cause language false-positive.",
...
describe(`real-LLM ingest scenarios (${scenarios.length})`, () => {
  for (const scenario of scenarios) {
    it.skipIf(!ENABLED)(
      `${scenario.name} (${scenario.targetLanguage})`,
      async () => {
        ctx = await setupScenario(scenario)
        const sourceFullPath = path.join(ctx.tmp.path, scenario.sourcePath)
        expect(
          await fileExists(sourceFullPath),
          `source fixture ${scenario.sourcePath} was not materialized`,
        ).toBe(true)
