  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        const firstRequest = reqs[0]
        if (firstRequest === undefined) throw new Error('expected one recorded request')
        expect(firstRequest.origin).toBe(server.url)
        // Body is the OpenAI-shape JSON we'd expect.
        const parsed: unknown = JSON.parse(firstRequest.body)
