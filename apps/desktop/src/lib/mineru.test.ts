...
import { mineruTest, parseWithMineru, parseWithMineruResult, testMineruConnection } from './mineru'

function uploadedForm(callIndex: number): FormData {
  const body = mockHttpFetch.mock.calls[callIndex]?.[1]?.body
  if (!(body instanceof FormData)) throw new Error(`expected a FormData upload body for call ${callIndex}`)
  return body
}
...
describe('MinerU API helpers', () => {
  it('maps official API error codes to actionable messages', () => {
    expect(mineruTest.mineruApiErrorMessage('A0202', 'bad token')).toContain('invalid')
    expect(mineruTest.mineruApiErrorMessage('A0211', 'expired')).toContain('expired')
    expect(mineruTest.mineruApiErrorMessage(-60005, 'too large')).toContain('200 MB')
    expect(mineruTest.mineruApiErrorMessage(-60006, 'too many pages')).toContain('200 page')
    expect(mineruTest.mineruApiErrorMessage(-60018, 'quota')).toContain('quota')
    expect(mineruTest.mineruApiErrorMessage(123, 'other')).toBe('MinerU API error 123: other')
  })
  ...
  it('prefers full.md from MinerU result zip', async () => {
    ...
    await expect(mineruTest.downloadAndExtractMarkdown('https://cdn/result.zip'))
  ...
  it('does not convert HTML tables inside fenced code blocks', async () => {
    ...
    const markdown = await mineruTest.downloadAndExtractMarkdown('https://cdn/result.zip')
  ...
  it('preserves and rewrites images inside MinerU HTML table cells', async () => {
    ...
    const markdown = await mineruTest.downloadAndExtractMarkdown(
  ...
  it('returns SavedImage metadata for MinerU zip images', async () => {
    ...
    const result = await mineruTest.downloadAndExtractMarkdownResult(
      'https://cdn/result.zip',
      undefined,
      { projectPath: '/project', sourceSummarySlug: 'paper' },
    )
  ...
  it('writes large extracted images with exact base64 content', async () => {
    ...
    await mineruTest.downloadAndExtractMarkdown(
      'https://cdn/result.zip',
      undefined,
      { projectPath: '/project', sourceSummarySlug: 'paper' },
    )
  ...
  it('keeps parsed Markdown when extracted image saving fails', async () => {
    ...
    await expect(mineruTest.downloadAndExtractMarkdown(
      'https://cdn/result.zip',
      undefined,
      { projectPath: '/project', sourceSummarySlug: 'paper' },
...
describe('parseWithMineru', () => {
  ...
  it('rejects unsupported MinerU model versions before reading or uploading', async () => {
    const persisted = JSON.parse('{"enabled":true,"token":"token","modelVersion":"mineru-html"}')
    await expect(parseWithMineru(persisted, '/tmp/doc.pdf')).rejects.toThrow('pipeline or vlm')
  ...
  it("rejects local files over MinerU's 200 MB accurate parsing limit before upload", async () => {
    fsMocks.getFileSize.mockResolvedValue(mineruTest.MAX_ACCURATE_PARSE_BYTES + 1)
  ...
  it('uploads the decoded PDF bytes to the MinerU upload URL', async () => {
    ...
    if (!(uploadBody instanceof ArrayBuffer)) throw new Error('expected an ArrayBuffer upload body')
  ...
  it('submits URL tasks without reading or uploading a local file', async () => {
    ...
    expect(fsMocks.getFileSize).not.toHaveBeenCalled()
    expect(fsMocks.readFileAsBase64).not.toHaveBeenCalled()
    const jsonBody = mockHttpFetch.mock.calls[0]?.[1]?.body
    if (typeof jsonBody !== 'string') throw new Error('expected a JSON string upload body')
