...
async function setup(scenario: typeof sweepScenarios[number]): Promise<Ctx> {
  ...
  useReviewStore.setState({
    items: scenario.reviews.map((r) => ({
      ...
      ...(r.affectedPages !== undefined ? { affectedPages: r.affectedPages } : {}),
      ...(r.searchQueries !== undefined ? { searchQueries: r.searchQueries } : {}),
    ...
    it.skipIf(!ENABLED)(
      ...
      async () => {
        ...
        for (const item of resolved) {
          const resolvedAction = item.resolvedAction
          expect(
            resolvedAction,
            `${item.id} resolved but has no resolvedAction`,
          ).toBeTruthy()
          expect(
            resolvedAction === 'auto-resolved' || resolvedAction === 'llm-judged',
            `${item.id} has unknown action: ${resolvedAction}`,
          ).toBe(true)
        }
        ...
        const partialResolution = scenario.name === 'mixed-batch/partial-resolution'
        ...
        expect(
          !partialResolution ? true : ruleResolved?.resolved,
          'r-mix-rule should ALWAYS be resolved by rule stage',
        ).toBe(true)
        expect(
          !partialResolution ? 'auto-resolved' : ruleResolved?.resolvedAction,
        ).toBe('auto-resolved')
        ...
        expect(
          !partialResolution ? false : contraItem?.resolved,
        ...
        expect(
          !partialResolution ||
            suggItem?.resolved !== true ||
            suggItem?.resolvedAction === 'llm-judged',
          'suggestion can only be resolved by LLM, not rules',
        ).toBe(true)

        const semanticMatch = scenario.name === 'llm-judged/semantic-match'
        ...
        for (const item of resolved) {
          expect(
            !semanticMatch || item.resolvedAction === 'llm-judged',
          ).toBe(true)
        }
      },
      TEST_TIMEOUT_MS,
