...
import { fetchImportUrl, MAX_BATCH_URLS, parseImportUrls, urlSourceFileName } from './url-source-import'
...
describe('fetchImportUrl', () => {
  it('blocks a public URL redirect before requesting a private target', async () => {
    const fetch = async (url: string | URL | Request, init?: RequestInit & { maxRedirections?: number }) => {
      expect(url instanceof Request ? url.url : url.toString()).toBe('https://example.com/start')
    ...
    await expect(
      fetchImportUrl(fetch, 'https://example.com/start', new AbortController().signal),
    )
  ...
  it('blocks IPv4-mapped IPv6 private redirect targets', async () => {
    ...
    await expect(fetchImportUrl(fetch, 'https://example.com', new AbortController().signal))
      .rejects.toThrow('cannot redirect')
  })
