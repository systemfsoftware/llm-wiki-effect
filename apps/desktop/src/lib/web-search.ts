...
import type { SearchApiConfig, SearchProviderConfigs, SearXngCategory, SerpApiEngine } from '@/stores/wiki-store'
...
export function resolveSearchConfig(config: SearchApiConfig): SearchApiConfig {
  ...
  const resolvedOllamaUrl = activeProvider === 'ollama'
...
export async function webSearch(
  ...
  if (
    ...
    throw new Error(
      'Web search not configured. Add an API key for the selected search provider in Settings, or select a key-free provider such as Firecrawl or SearXNG.',
    )
  }
