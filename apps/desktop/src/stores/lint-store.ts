...
function lintResultToItem(result: LintResult): LintItem {
  return {
    ...
    ...(result.affectedPages !== undefined ? { affectedPages: result.affectedPages } : {}),
    ...(result.brokenTarget !== undefined ? { brokenTarget: result.brokenTarget } : {}),
    ...(result.suggestedTarget !== undefined ? { suggestedTarget: result.suggestedTarget } : {}),
    ...(result.suggestedSource !== undefined ? { suggestedSource: result.suggestedSource } : {}),
    id: `lint-${++counter}`,
    createdAt: Date.now(),
  }
}
