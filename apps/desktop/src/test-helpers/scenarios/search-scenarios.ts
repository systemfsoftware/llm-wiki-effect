...
export const searchScenarios: SearchScenario[] = [
  ...
  //    should rank first and have titleMatch=true.
  {
    ...
    description: "Query 'attention'. attention.md has 'attention' in its title and in " +
  ...
  {
    name: 'content-match',
    description: "Query 'rotary' appears only in the body of embeddings.md, not in " +
  ...
  {
    name: 'cjk-bigram',
    description: "Query '注意力机制' is tokenized into bigrams ('注意', '意力', '力机', '机制'). " +
  ...
  // 4. multi-token-ranking — more matches ranks higher
  {
    ...
    description: "Query 'attention transformer'. The page mentioning BOTH terms " +
  ...
  // 5. stop-word-filtered — 'the' filtered, only meaningful tokens used
  {
    ...
    description: "Query 'the attention' should be tokenized to ['attention'] since " +
    ...
    initialWiki: {
      ...
      'wiki/other.md': page(
        'Other',
        "This page uses the word 'the' often but " +
  ...
  //    word many times. Regression guard for the "精准匹配排到最后" bug.
  {
    ...
    description: "Query 'attention'. Many pages contain 'attention' in title AND body. " +
  ...
  //    raw query phrase should outrank a page with the same tokens scattered.
  {
    ...
    description: "Query 'self-attention'. tokens split to ['self','attention']. " +
  ...
  //    related ones in the chat-context priority order.
  {
    ...
    description: "Query '总资产。' (with trailing 。). Page with '总资产' in BOTH title " +
