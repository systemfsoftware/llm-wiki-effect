...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      ...
      description:
        "Create or replace the vector index for one existing Markdown page under a project's wiki/ directory.",
      inputSchema: {
        type: 'object',
        properties: {
          project_id: { type: 'string', description: "Project UUID, project path, or 'current'. Defaults to current." },
          ...
          force: {
            type: 'boolean',
            description:
              'Force rebuilding vectors even when the page content and embedding configuration are unchanged.',
          },
        },
        required: ['path'],
        additionalProperties: false,
  ...
  try {
    switch (request.params.name) {
      ...
      case 'llm_wiki_projects': {
        ...
        return textResult(
          JSON.stringify({ ...(await client.projects()), sessionProject: projectBinding.project }, null, 2),
        )
      }
