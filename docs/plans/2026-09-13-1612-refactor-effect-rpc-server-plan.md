...
- KTD9. **House style per the bar repo; test stack per the repo's own TS precedent.** Code: `Context.Service` classes with `make`/`layer` (Effect v4 shape; `Effect.Service` banned — bar-repo oxlint plugin rule), `Schema.Class`/`Schema.TaggedError` with unique-symbol TypeIds, namespace barrels, Layer composition, one no-export entrypoint per process (the entry's execution is the declaration). Tests: vitest + fast-check + `@effect/vitest` for the new packages (repo precedent: `apps/desktop` runs vitest; the bar repo pairs vitest with Effect; property primitives via `effect/testing/FastCheck` per the v4 migration map). `apps/mcp-server` keeps its existing `node --test` harness — only its client swaps. Effect-specific lint presets are not adopted wholesale (lint-surface law). Gate: `pnpm lint` / `pnpm typecheck` green from U1 onward.
- KTD10. **Auth and gating as RPC middleware.** One middleware extracts the token (HTTP header standalone; handshake value on the socket), constant-time compares against the current store value on every operation (a rotated-away token must not survive on an open connection), and stamps an auth context. Per-operation requirements — chat/cancel/embed always token-required; MCP-mapped operations additionally check `mcpEnabled` → typed `McpDisabled` (the MCP-mapped set is marked in the operation catalog and asserted equal to the MCP tool surface in U5/U18) — are method-level middleware on the group. The MCP adapter keeps its pre-check for a good message. Rate limiting and body caps apply at the transport layer (both mounts). Gate: AE1/AE5 composition tests (U5).
