...
`version: true` opts the private root into the release plan, so
`pnpm release:version` bumps `package.json` and writes `CHANGELOG.md`. `tag:
false` keeps `changeset git-tag` away from the app: releases are tagged `v<version>`
by hand, because that is the shape `build.yml` triggers on.

**The app version has one source, and the copies are written by one script.**
`package.json` is the source. `src-tauri/tauri.conf.json` and
`src-tauri/Cargo.toml` carry copies, `src-tauri/Cargo.lock` records the crate
version, and `CHANGELOG` in `src/lib/changelog.ts` carries the version the app
displays. `scripts/sync-app-version.mjs` rewrites the first three from
`package.json`, and `pnpm release:version` runs it right after
`changeset version`, so the release step is one command plus the in-app
changelog entry.
...
rejected anyway: `src/lib/changelog.test.ts` asserts the manifest version equals
...
**One version source per artifact.** `package.json` is the app's. Anything that
needs the version reads it — `src-tauri/tauri.conf.json` by path, the release
workflow by syncing it into `extension/manifest.json` — rather than storing a
copy that a release has to remember to update.
...
synced 0.6.12 into src-tauri/tauri.conf.json, src-tauri/Cargo.toml, src-tauri/Cargo.lock
$ git diff --stat
package.json                | 2 +-
CHANGELOG.md                | 8 ++++++++
...
entry is the one step left to a human: `src/lib/changelog.test.ts` goes red until
