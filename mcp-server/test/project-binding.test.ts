...
void test('pin resolves current to a stable project id', () => {
...
void test('pinned sessions reject cross-project overrides', () => {
  const binding = new McpProjectBinding()
...
void test('unbound sessions preserve the current-project compatibility default', () => {
  const binding = new McpProjectBinding()
  assert.equal(binding.resolve(), 'current')
  assert.equal(binding.resolve('p2'), 'p2')
})

void test('responses carry a structural active-project reminder', () => {
