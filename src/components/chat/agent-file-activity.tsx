...
export function AgentFileActivity({
  ...
  const undo = async (change: ChatAgentFileChange) => {
    if (!project || change.beforeContent === undefined || change.afterContent === undefined || undone.has(change.id)) {
      return
    }
  ...
  return (
    <section
      className={embedded ? '' : 'rounded-md border border-border/60 bg-background/60'}
      aria-label={t('chat.agentChanges.title')}
    >
      {!embedded && (
          ...
          return (
            <div key={group.path} className='border-b border-border/40 last:border-b-0'>
              <div className='flex min-w-0 items-center gap-1.5 bg-muted/15 px-2 py-1.5 text-xs'>
                ...
                <span className='shrink-0 font-mono text-[10px] text-red-600 dark:text-red-400'>
                  -{group.deletions}
                </span>
              </div>
              {fileExpanded && group.edits.map((change, editIndex) => {
                const editExpanded = expandedEdits[change.id]
      ...
      {error && <div className='border-t border-destructive/20 px-2.5 py-1.5 text-[10px] text-destructive'>{error}
      </div>}
    </section>
  )
}
