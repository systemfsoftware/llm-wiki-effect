...
export function GeneralSection({ draft, setDraft }: Props) {
  ...
  const autostartLabel = t('settings.sections.general.autostart', { defaultValue: 'Launch at system startup' })
  ...
  return (
    <div className='space-y-6'>
      ...
      <label className='flex items-start gap-2' aria-label={autostartLabel}>
        <input
          type='checkbox'
      ...
      <div className='space-y-2'>
        ...
        <p className='text-xs text-muted-foreground'>
          {t('settings.sections.general.closeBehaviorHint', {
            defaultValue:
              'This setting applies when you click the title-bar close button. The tray menu can still quit the app directly.',
          })}
        </p>
      </div>
    </div>
