import { normalizeSourceWatchConfig, SOURCE_WATCH_FILE_TYPE_GROUPS } from '@/lib/source-watch-config'
...
export function SourceWatchSection({ draft, setDraft, projectReady }: Props) {
  ...
  const excludeDirsText = editingList === 'dirs' ? editingListText : joinList(config.excludeDirs)
  const excludeExtensionsText = editingList === 'extensions' ? editingListText : joinList(config.excludeExtensions)
  const excludeGlobsText = editingList === 'globs' ? editingListText : joinList(config.excludeGlobs)

  const updateConfig = (patch: Partial<typeof config>) => {
    setDraft('sourceWatchConfig', normalizeSourceWatchConfig({ ...config, ...patch }))
  }

  const beginListEdit = (field: 'dirs' | 'extensions' | 'globs', value: string) => {
    setEditingListText(value)
      ...
      <div className='space-y-4 rounded-lg border border-border/60 bg-muted/20 p-4'>
        <label className='flex items-start gap-3'>
          ...
          <span className='text-sm font-semibold'>
            <FolderSync className='mr-2 inline h-4 w-4 text-muted-foreground' />
            ...
            <span className='mt-1 block text-xs font-normal leading-relaxed text-muted-foreground'>
              {t('settings.sections.sourceWatch.enableDescription', {
                defaultValue: 'Refreshes the Sources view and indexes allowed new or modified files under raw/sources.',
              })}
            </span>
          </span>
        </label>
      ...
      <div className='space-y-4 rounded-lg border border-border/60 bg-muted/20 p-4'>
        ...
        <label className='block space-y-1.5'>
          ...
          <textarea
            ...
            onFocus={() => beginListEdit('dirs', joinList(config.excludeDirs))}
            onChange={(event) => {
              setEditingListText(event.target.value)
          ...
          <p className='text-[11px] leading-relaxed text-muted-foreground'>
            {t('settings.sections.sourceWatch.excludeDirsHint', {
              defaultValue:
                'Folder names match any path segment. Paths such as subdir/drafts match that nested folder.',
            })}
          </p>
        </label>
        ...
        <label className='block space-y-1.5'>
          ...
          <textarea
            ...
            onFocus={() => beginListEdit('extensions', joinList(config.excludeExtensions))}
            onChange={(event) => {
              setEditingListText(event.target.value)
        ...
        <label className='block space-y-1.5'>
          ...
          <textarea
            ...
            onFocus={() => beginListEdit('globs', joinList(config.excludeGlobs))}
            onChange={(event) => {
              setEditingListText(event.target.value)
