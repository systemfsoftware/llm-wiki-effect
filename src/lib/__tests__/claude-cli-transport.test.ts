...
import { buildExitError, createClaudeCodeStreamParser, streamClaudeCodeCli } from '../claude-cli-transport'
...
function readStreamId(payload: unknown): string {
  ...
  throw new Error('expected claude_cli_spawn payload with streamId')
}
...
describe('createClaudeCodeStreamParser', () => {
  ...
  it('emits only the novel tail when `assistant` events ship cumulative text', () => {
    ...
    const parse = createClaudeCodeStreamParser()
    const mk = (t: string) => JSON.stringify({ type: 'assistant', message: { content: [{ type: 'text', text: t }] } })
