 ...
 */
import { array, assert, integer, property, string } from 'fast-check'
...
describe('detectLanguage — properties', () => {
  it('pure ASCII random text always returns English', () => {
    ...
    assert(
      property(
        string({ minLength: 0, maxLength: 200 }).filter((s) => /^[\x20-\x7E]*$/.test(s)),
        (input) => {
  ...
  it('2+ CJK characters yield Chinese', () => {
    assert(
      property(
        array(
          integer({ min: 0x4E00, max: 0x9FFF }).map((cp) => String.fromCodePoint(cp)),
  ...
  it('2+ Hiragana characters yield Japanese', () => {
    assert(
      property(
        array(
          integer({ min: 0x3041, max: 0x3096 }).map((cp) => String.fromCodePoint(cp)),
          { minLength: 2, maxLength: 50 },
        ),
        (chars) => {
  ...
  it('2+ Hangul characters yield Korean', () => {
    assert(
      property(
        array(
          integer({ min: 0xAC00, max: 0xD7AF }).map((cp) => String.fromCodePoint(cp)),
          { minLength: 2, maxLength: 50 },
        ),
        (chars) => {
