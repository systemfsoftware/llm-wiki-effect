...
import { hasUsableLlm, type LlmProvider, PROVIDERS_WITHOUT_KEY } from './has-usable-llm'
...
describe('hasUsableLlm', () => {
  ...
  it('returns false instead of throwing when a key-required provider has a missing legacy key field', () => {
    const legacyConfig = { provider: 'azure' as const, apiKey: '' }
    Reflect.deleteProperty(legacyConfig, 'apiKey')
    expect(hasUsableLlm(legacyConfig)).toBe(false)
  })
