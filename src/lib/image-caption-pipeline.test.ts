...
const { mockCaption, mockReadBase64, mockReadFile, mockWriteFile, mockFileExists, mockCreateDir } = vi.hoisted(() => ({
  mockCaption: vi.fn<
    (
      imageBase64: string,
      mediaType: string,
  ...
  >(),
  mockReadBase64: vi.fn<(path: string) => Promise<{ base64: string; mimeType: string }>>(),
  mockReadFile: vi.fn<(path: string) => Promise<string>>(),
  mockWriteFile: vi.fn<(path: string, contents: string) => Promise<void>>(),
  mockFileExists: vi.fn<(path: string) => Promise<boolean>>(),
  mockCreateDir: vi.fn<(path: string) => Promise<void>>(),
}))
...
import type { CaptionOptions } from '@/lib/vision-caption'
import type { LlmConfig } from '@/stores/wiki-store'
import { captionInternals, captionMarkdownImages, loadCaptionCache } from './image-caption-pipeline'
...
describe('findImageReferences (helper)', () => {
  it('captures markdown image syntax with position info', () => {
    const refs = captionInternals.findImageReferences('text\n![](a.png)\n![label](b.jpg) more')
    expect(refs).toEqual([
...
describe('captionMarkdownImages', () => {
  ...
  it('does not reuse a caption cached for a different output language', async () => {
    mockReadBase64.mockResolvedValue({ base64: 'AAAA', mimeType: 'image/png' })
    mockCaption.mockResolvedValue('ein rotes Quadrat')
    const knownHash = await captionInternals.sha256OfBase64('AAAA')
  ...
  it('reuses a caption cached for the same output language', async () => {
    mockReadBase64.mockResolvedValue({ base64: 'AAAA', mimeType: 'image/png' })
    const knownHash = await captionInternals.sha256OfBase64('AAAA')
    const key = captionInternals.captionCacheKey(knownHash, 'German')
    mockFileExists.mockResolvedValue(true)
    mockReadFile.mockResolvedValue(JSON.stringify({
      [key]: {
        caption: 'ein rotes Quadrat',
...
describe('loadCaptionCache', () => {
  it('selects the newest caption per image when no language is requested', async () => {
    const knownHash = await captionInternals.sha256OfBase64('AAAA')
    const germanKey = captionInternals.captionCacheKey(knownHash, 'German')
    mockFileExists.mockResolvedValue(true)
