...
async function parseWithLocalMineru(
  ...
  const fileBuffer = bytesToUploadBody(bytes)
  ...
  const submitRes = await httpFetch(
    `${apiBase}/tasks`,
  ...
  const submitData: { task_id?: unknown } = await submitRes.json()
  ...
  while (Date.now() - start < LOCAL_POLL_TIMEOUT_MS) {
    ...
    if (status.status === 'completed') {
      ...
      if (assetOptions && first.images && typeof first.images === 'object') {
        const mediaDir = `${
          normalizePath(assetOptions.projectPath)
        }/wiki/media/${assetOptions.sourceSummarySlug}/mineru/images`
...
export async function testMineruConnection(
  ...
  if (config?.backend === 'local') {
    ...
    const health: { status?: unknown } | null = await res.json().catch(() => null)
