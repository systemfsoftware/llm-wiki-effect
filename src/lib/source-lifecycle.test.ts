...
const mocks = vi.hoisted(() => ({
  ...
  preprocessFile: vi.fn<(path: string) => Promise<string>>(),
  enqueueBatch: vi.fn<
    (projectId: string, files: Array<{ sourcePath: string; folderContext: string }>) => Promise<string[]>
  >(),
}))
...
describe('source import skip reporting', () => {
  ...
  function watchConfig(overrides: Partial<SourceWatchConfig> = {}): SourceWatchConfig {
    return {
      enabled: true,
      autoIngest: true,
      persistExtractedMarkdown: false,
  ...
  it('reports files a folder import left behind', async () => {
    ...
    mocks.getFileSize.mockImplementation(async (path: string) => path.endsWith('huge.pdf') ? 149696996 : 1024)
