...
describe('real-LLM sweep chained (ingest → ingest → sweep)', () => {
  it.skipIf(!ENABLED)(
    ...
    async () => {
      ...
      const hasLayerNormPage = secondWritten.some((p) => /layer[-_]?norm|normalization/i.test(p.toLowerCase()))
      expect(
        hasLayerNormPage,
      ...
      expect(
        ...
        'sweep should have resolved at least one missing-page review after its concept page was created in step 2',
      ).toBeGreaterThanOrEqual(1)
