...
import { type Chunk, chunkMarkdown, stripFrontmatter } from './text-chunker'
...
describe('chunkMarkdown — realistic wiki fixture', () => {
  ...
  it('each chunk is ≤ maxChars (or flagged oversized)', () => {
    const result = chunkMarkdown(wikiPage)
    const sizedChunks = result.filter((c) => !c.oversized)
    expect(sizedChunks.length).toBeGreaterThan(0)
