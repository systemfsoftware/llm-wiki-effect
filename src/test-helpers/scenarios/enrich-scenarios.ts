...
export const enrichScenarios: EnrichScenario[] = [
  ...
  {
    name: 'adds-wikilinks',
    description: 'LLM returns a JSON list identifying Transformer and Attention as ' +
    ...
    initialWiki: {
      ...
      'wiki/survey.md': '# Deep Learning Survey\n\n' +
    ...
    expected: {
      ...
      expectedContent: '# Deep Learning Survey\n\n' +
        'Modern NLP relies on [[Transformer]] architectures for most tasks. ' +
        'The Transformer was introduced in 2017 and has since dominated. ' +
        '[[Attention]] is the key mechanism that makes it work.\n',
    },
  ...
  // 2. preserves-frontmatter — YAML block is never touched by code
  {
    ...
    description: 'Frontmatter preservation is now a code-level guarantee: the ' +
  ...
  // 3. no-matches-no-write — LLM returns empty list; nothing written
  {
    ...
    description: 'LLM returns {links:[]} — nothing to substitute. writeFile is not ' +
      'called; the file stays unchanged.',
    initialWiki: {
      'wiki/index.md': WIKI_INDEX_WITH_TRANSFORMER,
      'wiki/unrelated.md': '# Unrelated Page\n\n' +
  ...
  {
    name: 'cjk-terms',
    description: 'Chinese content; LLM returns list with Chinese term / target. ' +
